...
4. **Report.** The stray's account comes from its `account` field (show `—` if none) — a bookkeeper needs it to find the row in their app:

   | Payee | Usual category (share) | Stray transaction | Account | Its category | Verdict | Suggested fix |
